...
export default async function CityPage({
  ...
  // 1. Fetch Data from The Vault
  const locations = getLocations();
  const stateData = locations.find((s: any) => s.slug === stateSlug);
  const cityData = stateData?.cities.find((c: any) => c.slug === citySlug);
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <nav className="text-sm text-gray-500 font-medium">
            ...
            <span className="mx-2">/</span>
            <span className="text-gray-900">{cityData.name}, {stateData.abbr}</span>
          </nav>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12 grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-2">
          ...
          <div className="prose prose-lg text-gray-600 mb-10 max-w-none">
            <p className="mb-4">
              ServicePro is the leading appliance repair service in {cityData.name}. We repair most major appliance brands, makes and models, no matter where you bought them. Our goal is to provide quality repairs to help you extend the useful life of your household appliances.
            </p>
            <p className="mb-4">
              Our technicians are local to your neighborhood in <strong>{stateData.name}</strong> and you can easily schedule your appointment online or over the phone. ServicePro is the only nationally authorized appliance repair service for Kenmore products.
            </p>
            <p className="mb-6">
              We have performed millions of repairs on refrigerators, washers, dryers, ranges and more. Our technicians are experts who you can trust for repairs on top brands like Samsung, Maytag, Bosch and LG.
            </p>
            
            <div className="bg-brand-dark/5 p-6 rounded-xl border-l-4 border-brand-red">
              <p className="font-bold text-brand-dark m-0">
                Are you in need of appliance repair in {cityData.name}?
              </p>
              <p className="mt-2 m-0">
                Call the ServicePro experts or schedule your appointment online today.
              </p>
            </div>
          </div>
          ...
          {/* Branch List (The Database Content) */}
          <h2 className="text-2xl font-bold text-brand-dark mb-6 border-t pt-8">Local Service Centers</h2>
          <div className="space-y-4">
            {cityData.branches && cityData.branches.length > 0 ? (
              cityData.branches.map((branch: any) => (
                <div key={branch.slug} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:border-brand-red/30 transition-all">
                  <div>
                    <h3 className="font-bold text-lg text-gray-900">{branch.address}</h3>
                    <p className="text-gray-500 text-sm">ServicePro Appliance Repair • {cityData.name}</p>
                    <div className="mt-2 text-xs text-green-600 font-bold uppercase tracking-wide flex items-center gap-1">
                      <span className="w-2 h-2 rounded-full bg-green-500"></span> Open Now
                    </div>
                  </div>
                  <Link 
                    href={`/locations/${stateSlug}/${citySlug}/servicepro-appliance-repair/${branch.slug}`}
                    className="bg-gray-100 text-brand-dark px-6 py-3 rounded-xl text-sm font-bold hover:bg-brand-red hover:text-white transition-all text-center shadow-sm whitespace-nowrap"
                  >
                    View Details
                  </Link>
                </div>
               ...
               <div className="p-8 bg-white rounded-2xl text-center border border-dashed border-gray-300">
                 <p className="text-gray-500 italic mb-4">We serve the entire {cityData.name} area via central dispatch.</p>
                 <a href="tel:800-665-2127" className="inline-block bg-brand-dark text-white px-6 py-2 rounded-lg font-bold text-sm">Call Dispatch</a>
               </div>
            )}
          </div>
        </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white p-8 rounded-3xl shadow-2xl shadow-gray-200/50 border border-gray-100 sticky top-24">
            ...
            <button className="w-full bg-brand-red text-white font-bold py-4 rounded-xl shadow-lg hover:bg-red-700 hover:shadow-xl transition-all mb-4 transform active:scale-[0.98]">
              Book Appointment
            </button>
            <a href="tel:8006652127" className="block w-full text-center bg-white border-2 border-gray-100 text-gray-700 font-bold py-3 rounded-xl hover:border-brand-dark hover:text-brand-dark transition-colors">
              Call (800) 665-2127
            </a>
            
            <div className="mt-6 pt-6 border-t border-gray-100 text-center flex justify-center items-center gap-2">
               <span className="text-lg">🛡️</span>
               <span className="text-xs text-gray-400 font-medium">100% Satisfaction Guaranteed</span>
            </div>
          </div>
        </div>
